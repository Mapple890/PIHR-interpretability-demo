...
# ============================================================
# PART 2: DEFINITIVE PIHR Architecture 
# ============================================================
...
class ModuleC(nn.Module):
    def __init__(self, input_dim=64, model_dim=128, num_heads=8, num_layers=4):
        super().__init__()
        # Input projection is now simpler
...
class PIHR(nn.Module):
    def __init__(self, input_channels=8, num_classes=3, time_steps=144):
        ...
        # Note ModuleC's signature has changed
    ...
    def forward(self, x, derivatives):
        ...
        # Pass only X_st to Module C
        X_st_prime = self.module_c(X_st)
        # Module D still uses derivatives for instability score
        ...
        logits = self.classification_head(H_enhance)
        return logits
...
def train_model(model, train_loader, epochs=60): # Increased epochs slightly
    model.to(device)
    ...
    model.train()
    print("Training PIHR on a realistic challenge (no derivative shortcut)...")
    for epoch in range(epochs):
        ...
        for batch_data, batch_derivatives, batch_labels in train_loader:
            ...
            if num_to_sim > 0:
                p2_data, p2_deriv, _ = zip(*[generate_thickener_data('p2') for _ in range(num_to_sim)])
                p2_data = torch.from_numpy(np.array(p2_data)).to(device)
                p2_deriv = torch.from_numpy(np.array(p2_deriv)).to(device)
                ds = train_loader.dataset
                p2_data = (p2_data - ds.mean_data_tensor) / (ds.std_data_tensor + 1e-6)
                p2_deriv = (p2_deriv - ds.mean_deriv_tensor) / (ds.std_deriv_tensor + 1e-6)
                p2_logits = model(p2_data, p2_deriv)
            ...
            optimizer.step()
            total_ce_loss += ce_loss.item(); total_sim_loss += sim_loss.item()
        scheduler.step()
        avg_ce = total_ce_loss / len(train_loader); avg_sim = total_sim_loss / len(train_loader)
        if (epoch + 1) % 5 == 0:
...
def demonstrate_pihr():
    print("="*60 + "\nPIHR DEMONSTRATION: REALISTIC CHALLENGE VERSION\n" + "="*60)
    ...
    if p2_accuracy > 85: 
        ...
        print("This demonstrates the model's ability to learn complex temporal dynamics from raw multi-variate data,")
        print("without relying on derivative shortcuts, which is a more robust and realistic achievement.")
    else:
        print(f"The model achieved {p2_accuracy:.1f}% accuracy on the unseen class. This score reflects a realistic learning challenge.")
    print("="*60)
